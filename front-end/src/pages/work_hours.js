...
export default function Working_hours() {
  ...
  const [data_workinghours,setData_workinghours] = useState([])
  const [currentPage, setCurrentPage] = useState(1);
  const [currentData,setCurrentData] = useState([]);
  const maxdata = 50
  const maxPages = Math.ceil(data_workinghours.length / maxdata);
  ...
  async function get_data_workinghours(){
    setLoding(true)
    await axios.get('http://localhost:8000/api/get_workinghours').then(({data})=>{
        setData_workinghours(data.data);setLoding(false)
    }).catch(({})=>{
      Error_t()
      setLoding(false)
    })
    ...
    useEffect(()=>{
        get_data_workinghours()
    },[done])


    function handler_editData(Date,StartTimeN,EndTimeN,StartTimeE,EndTimeE) {
      if (StartTimeE == '00:00:00' || EndTimeE == '00:00:00') {
        StartTimeE = '' 
        EndTimeE = ''
      }
      setData({Date,StartTimeN,EndTimeN,StartTimeE,EndTimeE})
  }
  ...
  async function handler_SendEdit() {
    setLodingbtn(true)
    await axios.post(`http://localhost:8000/api/update_workinghours/${id}`,data).then(({data})=>{
  ...
  async function del_data() {
    await axios.get(`http://localhost:8000/api/del_shift/${id}`).then(({data})=>{
  ...
  const handleNextPage = () => {
    ...
    if (searchQuery != '' || searchQueryDate != '' ) {
        const filteredData = data_workinghours.filter((item) => {
          ...
          const dateMatch = searchQueryDate ? item.Date === searchQueryDate : true;
    ...
    useEffect(()=>{
      if (searchQuery === '' && searchQueryDate === '') {
        setCurrentData(data_workinghours.slice(startIndex, endIndex))
      }
     },[searchQuery,data_workinghours,currentPage,searchQueryDate])
...
<span class="text-gray-700  dark:text-gray-400">Sélectionner une date</span>
  <input  type="date" class="bg-gray-50 mt-2  border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
   placeholder="Select date" onChange={(e)=>setSearchQueryDate(e.target.value)}/>
  </label>
  <label class="block text-sm ">
              <span class="text-gray-700 dark:text-gray-400">Nom de l'employé</span>
              <input
                class="bg-gray-50 mt-2 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
            ...
            <table class="w-full whitespace-no-wrap">
              <thead>
                ...
                >
                  ...
                  <th class="px-4 py-3">Nom de l'employé</th>
                  <th class="px-4 py-3">Date</th>
                  <th class="px-4 py-3">Heure de début (regular)</th>
                  <th class="px-4 py-3">Heure de fin (regular)</th>
                  <th class="px-4 py-3">Heure de début (overtime)</th>
                  <th class="px-4 py-3">Heure de fin (overtime)</th>
                  <th class="px-4 py-3">regular(H)</th>
                  <th class="px-4 py-3">overtime(H)</th>
...
<tr class="text-gray-700 dark:text-gray-400">
                <td class="px-4 py-3 text-sm ">
                  ...
                  <td class="px-4 py-3 text-sm">
                  {item.StartTimeE != '00:00:00' ? item.StartTimeE : '----------' }
                  </td>
                  <td class="px-4 py-3 text-sm">
                  {item.EndTimeE != '00:00:00' ? item.EndTimeE : '----------' }
                  </td>
                  <td class="px-4 py-3 text-sm">
                  { ConvertDecimalToTime( item.hoursWorkedN )} 
                  </td>
                  <td class="px-4 py-3 text-sm">
                  {item.hoursWorkedE == 0 ? '------' : ConvertDecimalToTime(item.hoursWorkedE)} 
                  </td>
                  <td class="px-4 py-3">
                    <div class="flex items-center space-x-4 text-sm">
                      <button
                        ...
                        aria-label="Edit"
                        onClick={()=>{setModuleedit(true);setId(item.ShiftID)
                          handler_editData(item.Date,item.StartTimeN,item.EndTimeN,item.StartTimeE,item.EndTimeE)
                        }}
                      >
                        <svg
                          class="w-5 h-5"
                      ...
                      <button
                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-blue-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                        aria-label="Delete"
                        onClick={()=>{setModule(true);setId(item.ShiftID)}}
                      >
                        <svg
                          class="w-5 h-5"
                          aria-hidden="true"
...
>
Modification des données sur le temps de travail du salarié
</p>
  ...
  <label class="block text-sm mt-4">
    ...
    />
     <span class="text-xs text-red-600 dark:text-red-400">
                  {err.hasOwnProperty('Date') ? err.Date[0] : ''}
                </span>
  </label>
  ...
  <label class="block text-sm">
    <span class="text-gray-700 dark:text-gray-400">HEURE DE DÉBUT RÉGULIÈRE</span>
    <input
      class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
      type='time'
      name="StartTimeN"
      value={data.StartTimeN}
      onChange={(e)=>handlergetdata(e)}
    />
    <span class="text-xs text-red-600 dark:text-red-400">
                  {err.hasOwnProperty('StartTimeN') ? err.StartTimeN[0] : ''}
                </span>
  </label>
  <label class="block text-sm">
    <span class="text-gray-700 dark:text-gray-400">HEURE DE FIN RÉGULIÈRE</span>
    <input
      class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
      type='time'
      name="EndTimeN"
    ...
    />
    <span class="text-xs text-red-600 dark:text-red-400">
                  {err.hasOwnProperty('EndTimeN') ? err.EndTimeN[0] : ''}
                </span>
  </label>
  </div>
...
<div className="grid grid-cols-2 gap-6 mt-4">
 ...
 <label class="block text-sm">
   <span class="text-gray-700 dark:text-gray-400">HEURE DE DÉBUT DES HEURES SUPPLÉMENTAIRES</span>
   <input
     class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
     type='time'
     name="StartTimeE"
     value={data.StartTimeE}
     onChange={(e)=>handlergetdata(e)}
   />
   <span class="text-xs text-red-600 dark:text-red-400">
                 {err.hasOwnProperty('StartTimeE') ? err.StartTimeE[0] : ''}
               </span>
 </label>
 <label class="block text-sm">
   <span class="text-gray-700 dark:text-gray-400">HEURE DE FIN DES HEURES SUPPLÉMENTAIRES &nbsp;</span>
   <input
     class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-blue-400 focus:outline-none focus:shadow-outline-blue dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
     type='time'
     name="EndTimeE"
   ...
   />
   <span class="text-xs text-red-600 dark:text-red-400">
                 {err.hasOwnProperty('EndTimeE') ? err.EndTimeE[0] : ''}
               </span>
 </label>
 </div>
 <label class="text-sm mt-4">
 <button
  class="w-full px-5 mt-10 py-3 text-sm font-medium leading-5 text-black text-gray-700 transition-colors duration-150 underline rounded-lg dark:text-gray-400 sm:px-4 sm:py-2 sm:w-auto active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray"
  onClick={() => setData({...data , 'StartTimeE' : '' , 'EndTimeE' : ''  })}
  >
Rien
</button></label>
  </div>
